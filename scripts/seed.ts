 ...
 *
 * Opens its own connection rather than importing the app's client, which is
 * marked server-only.
 */
...
import * as a from "../src/db/schema/allocation.ts";
import * as p from "../src/db/schema/people.ts";
...
applyPragmas(sqlite);
const adb = drizzle(sqlite, { schema: { ...a, ...p } });

const pdb = adb;

console.log("Clearing existing data…");
// Children before parents: the people tables now hold real foreign keys into
// `users`, so clearing in the wrong order is a constraint error rather than a
// silent orphan.
sqlite.exec(`
  ...
  .values([
    {
      ...
      notes: "Client-facing. Peak load at quarter end.",
    },
    {
      name: "Payments Gateway",
      requiredCapacity: 150,
      notes: "PCI scope. Changes need a second reviewer.",
    },
    {
      name: "Risk Data Pipeline",
      requiredCapacity: 100,
      notes: "Overnight batch. On-call rotation applies.",
    },
    {
      name: "Client Onboarding",
      requiredCapacity: 250,
      notes: "Largest delivery commitment this year.",
    },
    {
      name: "Internal Tooling",
      requiredCapacity: 50,
      notes: "Best-effort. First to lose people when priorities move.",
    },
  ])
  .returning()
  .all();
...
const userRows = adb
  ...
  .returning()
  .all();
const [amara, bea, callum, dilnoza, emeka, farida, gustav, hina, ingrid] =
  userRows;
...
const seeds: Seed[] = [
  // Amara — full at 100%, with a closed spell on Onboarding behind her.
  {
    ...
    startDate: months(-14),
    endDate: months(-6),
    note: "Initial onboarding build-out",
  },
  {
    userId: amara.id,
    appId: ratings.id,
    ...
    startDate: months(-6),
    endDate: null,
    note: "Moved to Ratings after onboarding MVP shipped",
  },
  ...
  // Bea — 120% today. Over-committed, and it started when the pipeline work landed.
  {
    ...
    startDate: months(-9),
    endDate: null,
    note: "Gateway maintenance and PCI remediation",
  },
  {
    userId: bea.id,
    appId: pipeline.id,
    ...
    startDate: months(-2),
    endDate: null,
    note: "Pulled in to unblock the overnight batch rewrite",
  },
  ...
  // Dilnoza — under-allocated at 40%, with more planned from next month.
  {
    ...
    startDate: months(-4),
    endDate: null,
    note: "Ramping up after joining the team",
  },
  {
    userId: dilnoza.id,
    appId: pipeline.id,
    ...
    startDate: months(1),
    endDate: null,
    note: "Planned: second app once onboarding ramp completes",
  },
  ...
  // Emeka — UNALLOCATED today. Came off the gateway three weeks ago.
  {
    ...
    startDate: months(-16),
    endDate: days(-21),
    note: "Rolled off after the gateway migration completed",
  },
  ...
  // Farida — full.
  {
    ...
    startDate: months(-6),
    endDate: null,
    note: "Joined the onboarding delivery squad",
  },
  ...
  },
  {
    ...
    startDate: days(42),
    endDate: null,
    note: "Planned: moves to Ratings once the rewrite ships",
  },
...
const oneOnOneRows = pdb
  ...
  .values([
    ...
    },
    {
      ...
      theirTopics:
        "Bus-factor concern on exports. Suggested pairing with Farida to spread it.",
      createdAt: now(),
      updatedAt: now(),
    },
  ])
...
pdb
  .insert(p.actionItems)
  .values([
    {
      userId: bea.id,
      oneOnOneId: beaRecent.id,
      description:
        "Review Bea's split across Gateway and Pipeline — she is at 120%",
      owner: "manager",
      status: "open",
      dueDate: days(3),
      createdAt: now(),
    },
    {
      ...
      oneOnOneId: beaRecent.id,
      description:
        "Write up what 'senior' looks like against the current framework",
      owner: "manager",
      status: "open",
      dueDate: days(10),
      createdAt: now(),
    },
    {
      userId: bea.id,
      oneOnOneId: beaRecent.id,
      description: "Draft the design doc for the batch rewrite phase 2",
      owner: "report",
      status: "open",
      dueDate: days(14),
      createdAt: now(),
    ...
    {
      userId: emeka.id,
      oneOnOneId: emekaLast.id,
      description: "Confirm Emeka's next assignment — OVERDUE",
      owner: "manager",
      status: "open",
      dueDate: days(-10),
      createdAt: now(),
    ...
    {
      userId: gustav.id,
      oneOnOneId: gustavLast.id,
      description: "Find Gustav a mentor outside the team",
      owner: "manager",
      status: "open",
      dueDate: days(21),
      createdAt: now(),
    ...
    {
      userId: amara.id,
      oneOnOneId: amaraLast.id,
      description: "Set up Amara/Farida pairing on the export pipeline",
      owner: "manager",
      status: "done",
      createdAt: now(),
      closedAt: now(),
...
const goalRows = pdb
  ...
  .values([
    {
      userId: bea.id,
      title: "Lead a design-led piece of work end to end",
      detail:
        "Own the design, not just the implementation, for a project with real ambiguity in it.",
      category: "technical",
      status: "active",
      targetDate: months(4),
      createdAt: now(),
    ...
    {
      userId: bea.id,
      title: "Mentor one engineer through their first quarter",
      category: "leadership",
      status: "active",
      targetDate: months(6),
      createdAt: now(),
    ...
    {
      userId: amara.id,
      title: "Spread export-pipeline knowledge to at least two others",
      category: "delivery",
      status: "active",
      targetDate: months(3),
      createdAt: now(),
    ...
    {
      userId: amara.id,
      title: "Run the quarter-end readiness review",
      category: "leadership",
      status: "achieved",
      createdAt: now(),
      updatedAt: now(),
    ...
    {
      userId: gustav.id,
      title: "Ship a change to every service in the platform group",
      detail:
        "A breadth exercise — the point is exposure, not the size of the changes.",
      category: "technical",
      status: "active",
      targetDate: months(5),
      createdAt: now(),
    ...
    {
      userId: emeka.id,
      title: "Move into a delivery-facing role with a fixed end date",
      category: "delivery",
      status: "paused",
      createdAt: now(),
      updatedAt: now(),
...
pdb
  .insert(p.goalUpdates)
  .values([
    {
      goalId: beaDesignGoal.id,
      date: days(-35),
      note: "Batch rewrite identified as the candidate piece.",
    },
    {
      goalId: beaDesignGoal.id,
      date: days(-7),
      note: "Started, but the 120% allocation is squeezing the design time out of it.",
    },
    {
      goalId: amaraExports.id,
      date: days(-16),
      note: "Pairing with Farida agreed. Not started yet.",
    },
    // Deliberately stale: no update in over 60 days, so §5.2 flags it as quietly stalling.
    {
      goalId: gustavBreadth.id,
      date: days(-74),
      note: "Covered three of the seven services so far.",
    },
  ])
  .run();
  ...
  .values([
    {
      ...
      date: days(-5),
      direction: "received",
      source: "Payments tech lead",
      category: "praise",
      content:
        "Unblocked the batch rewrite in a week after it had been stuck for a month. Clear written summary afterwards.",
      shared: false,
    },
    {
      userId: bea.id,
      ...
      content:
        "Discussed picking up work before the current piece is finished — we agreed she would flag capacity before saying yes.",
      shared: true,
    },
    {
      userId: amara.id,
      date: days(-9),
      direction: "received",
      source: "Client onboarding stakeholder",
      category: "praise",
      content:
        "Handover notes on the onboarding MVP were unusually thorough and saved the receiving team a lot of time.",
      shared: false,
    },
    {
      userId: gustav.id,
      ...
      category: "praise",
      content:
        "Ramp-up has been faster than expected. Said so directly in our 1:1.",
      shared: true,
    },
    {
      userId: farida.id,
      date: days(-2),
      direction: "received",
      source: "Onboarding squad peer",
      category: "praise",
      content:
        "Picked up the export pipeline context quickly and started documenting it unprompted.",
      shared: false,
    },
  ])
  .run();
...
for (const t of [
  ...
  console.log(`  ${t.padEnd(20)} ${count(sqlite, t)}`);
}
